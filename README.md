...
Create a single `.env` in the repo root. Both the token server and the agent read from it:
...
### 3) Local LiveKit and Weaviate (Docker)
...
```
uv run python agent/voice_agent_realtime.py dev
```
